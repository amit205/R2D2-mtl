...
class PairLoader:
    ...
    def __getitem__(self, i):
        ...
        img_a, img_b, metadata, semanticMask = self.dataset.get_pair(i, self.what)
        ...
        output_size_semantics = min(semanticMask.size, crop_size)
        img_a = np.array(img_a)
        img_b = np.array(img_b)
        scaled_semantics = np.array(semanticMask)
        scaled_semantics = scaled_semantics/255
        ...
        if None in best: # counldn't find a good window
            ...
            scaled_semantics = np.zeros(output_size_a[::-1]+(3,), dtype=np.uint8)
            ...
            img_a = img_a[win1]
            img_b = img_b[win2]
            scaled_semantics = scaled_semantics[win1]
            ...
            if img_a.shape[:2][::-1] != output_size_a:
                ...
                scaled_semantics = np.asarray(Image.fromarray(scaled_semantics).resize(output_size_a, Image.ANTIALIAS))
        ...
        if 'flow' in self.what:
            ...
            flow = aflow - mgrid
        result = dict(img1=self.norm(img_a), img2=self.norm(img_b), semanticMask1 = scaled_semantics)
